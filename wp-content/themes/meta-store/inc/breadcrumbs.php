<?php
if( !function_exists( 'meta_store_breadcrumb' ) ) {
    function meta_store_breadcrumb($args = array()) {

        $breadcrumb = apply_filters('meta_store_breadcrumb_object', null, $args);

        if (!is_object($breadcrumb))
            $breadcrumb = new Meta_Store_Breadcrumb($args);
...
class Meta_Store_Breadcrumb {
    ...
    public function __construct($args = array()) {
        ...
        $this->args = apply_filters('meta_store_breadcrumb_args', wp_parse_args($args, $defaults));
    ...
    public function trail() {
        ...
        // Connect the breadcrumb if there are items in the trail.
        if (0 < $item_count) {
            ...
            if (true === $this->args['show_browse']) {
                ...
                $breadcrumb .= sprintf(
                        '<%1$s class="trail-browse">%2$s</%1$s>', tag_escape($this->args['browse_tag']), wp_kses_post($this->labels['browse'])
                );
            }
            ...
            // Wrap the breadcrumb.
            $breadcrumb = sprintf(
                    '<%1$s role="navigation" aria-label="%2$s" class="ms-breadcrumb-trail ms-breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>', tag_escape($this->args['container']), esc_attr($this->labels['aria_label']), $this->args['before'], $breadcrumb, $this->args['after']
            );
        }

        // Allow developers to filter the breadcrumb HTML.
        $breadcrumb = apply_filters('meta_store_breadcrumb', $breadcrumb, $this->args);
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse' => esc_html__('Browse:', 'meta-store'),
            'aria_label' => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'meta-store'),
            'home' => esc_html__('Home', 'meta-store'),
            'error_404' => esc_html__('404 Not Found', 'meta-store'),
            'archives' => esc_html__('Archives', 'meta-store'),
            // Translators: %s is the search query.
            'search' => esc_html__('Search results for: %s', 'meta-store'),
            // Translators: %s is the page number.
            'paged' => esc_html__('Page %s', 'meta-store'),
            // Translators: %s is the page number.
            'paged_comments' => esc_html__('Comment Page %s', 'meta-store'),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute' => esc_html__('Minute %s', 'meta-store'),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week' => esc_html__('Week %s', 'meta-store'),
        ...
        $this->labels = apply_filters('meta_store_breadcrumb_labels', wp_parse_args($this->args['labels'], $defaults));
    }
    ...
    protected function set_post_taxonomy() {
        ...
        $this->post_taxonomy = apply_filters('meta_store_breadcrumb_post_taxonomy', wp_parse_args($this->args['post_taxonomy'], $defaults));
    }
    ...
    protected function add_items() {
        ...
        $this->items = array_unique(apply_filters('meta_store_breadcrumb_items', $this->items, $this->args));
    }
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'meta-store')));
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'meta-store')));
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if (true === $this->args['show_title'])
            $this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'meta-store')));
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'meta-store')));
        $month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'meta-store')));
        $day = sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'meta-store')));
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'meta-store')));
        $week = sprintf($this->labels['archive_week'], get_the_time(esc_html_x('W', 'weekly archives date format', 'meta-store')));
    ...
    /**
     * Turns %tag% from permalink structures into usable links for the breadcrumb.  This feels kind of
    ...
    protected function map_rewrite_tags($post_id, $path) {
        ...
        if (is_array($matches)) {
            ...
            foreach ($matches as $match) {
                ...
                if ('%year%' == $tag)
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'meta-store'))));

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ('%monthnum%' == $tag)
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'meta-store'))));

                // If using the %day% tag, add a link to the daily archive.
                elseif ('%day%' == $tag)
                    $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'meta-store'))));
